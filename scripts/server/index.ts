import * as express from 'express';
import { Request, Response, NextFunction } from 'express';
import * as bodyParser from 'body-parser';
import { S3 } from 'aws-sdk';
import * as multer from 'multer';
import * as path from 'path';
import * as fs from 'fs';

const fileTypeFromBase64Image = function(base64Image: string): string {
  switch (base64Image.charAt(0)) {
    case '/':
      return 'jpeg';
    case 'i':
      return 'png';
    case 'R':
      return 'gif';
    case 'U':
      return 'webp';
    default:
      return null;
  }
};

const app = express();

app.use(
  bodyParser.urlencoded({
    extended: true
  })
);

app.use(bodyParser.json());

app.use((_: Request, res: Response, next: NextFunction) => {
  res.header('Access-Control-Allow-Origin', '*');
  res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
  next();
});

const uploadDirectory = path.join(__dirname, 'upload');

const dir = multer({ dest: uploadDirectory });

app.get('/', (_: Request, res: Response) => {
  return res.send('Hello world');
});

app.post('/presigned', (req: Request, res: Response) => {
  console.log('req.body.key', req.body.key);
  const key = req.body.key;
  const params = {
    Bucket: 'react-redux-ts-sandbox',
    Key: key,
    Expires: 60,
    ContentType: 'image/png'
  };
  const client = new S3({ apiVersion: '2006-03-01' });
  client.getSignedUrl('putObject', params, (_, signedUrl) => {
    console.info('signedUrl', signedUrl);
    return res.send({ url: signedUrl });
  });
});

// curl -X POST -H "Content-Type: application/json" -d '{"image":""}' localhost:8080/base64
app.post('/base64', async (req: Request, res: Response) => {
  console.log('req.body.image', req.body.image);
  const base64Image = req.body.image;
  const decode = new Buffer(base64Image, 'base64');
  const type = fileTypeFromBase64Image(base64Image);
  const client = new S3({ apiVersion: '2006-03-01' });
  const key = new Date().getTime();
  const uploadParams = {
    Bucket: 'react-redux-ts-sandbox',
    Key: `${key}.${type}`,
    Body: decode,
    ContentType: `image/${type}`
  };
  await client.upload(uploadParams).promise();
  return res.send('base64 upload success');
});

app.post('/fileUpload', dir.single('uploadFile'), async (req: Request, res: Response) => {
  console.log('filename: ' + req.file.originalname);
  console.log('uploaded path: ' + req.file.path);
  console.log('stored path: ' + req.file.filename);
  fs.readFile(req.file.path, 'base64', async (err, data) => {
    if (err) {
      console.log('read file err', err);
    }
    const decode = new Buffer(data, 'base64');
    const type = fileTypeFromBase64Image(data);
    const client = new S3({ apiVersion: '2006-03-01' });
    const key = new Date().getTime();
    const uploadParams = {
      Bucket: 'react-redux-ts-sandbox',
      Key: `${key}.${type}`,
      Body: decode,
      ContentType: `image/${type}`
    };
    await client.upload(uploadParams).promise();
    return res.send('fileUpload success');
  });
});

app.listen(8080, () => {
  console.log('Local Server app listening on port 8080');
});

export default app;
